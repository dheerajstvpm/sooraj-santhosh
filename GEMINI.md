...
* **Package Management:** The AI will use `pnpm` or `yarn` for package management.
* **Next.js CLI:** The AI will use the Next.js CLI for common development tasks:
  ...
  * `pnpm run lint`: To run ESLint and check for code quality issues.
...
* **Post-Modification Checks:** After every code modification, the AI will:
  * Run `pnpm run lint -- --fix` to catch and fix linting issues.
...
* **Error Checking Flow:**
  ...
  4. **Compile & Analyze:** AI runs `pnpm run lint` and monitors the dev server.
